...
const FEATURES = [
  { icon: "⚡", title: "Instant Transfers", desc: "Send money across 50+ countries in under 3 seconds. No delays, no excuses." },
  { icon: "🔒", title: "Bank-Grade Security", desc: "256-bit encryption, biometric auth, and real-time fraud detection on every transaction." },
  { icon: "💱", title: "Live FX Rates", desc: "Exchange at interbank rates with zero hidden markups. What you see is what you pay." },
  { icon: "📊", title: "Spend Analytics", desc: "Visualise where your money goes with intelligent categorisation and monthly reports." },
  { icon: "🌍", title: "Multi-Currency Wallets", desc: "Hold, convert, and spend in USD, EUR, GBP, NGN and 30+ currencies simultaneously." },
  { icon: "🤝", title: "Business API", desc: "Integrate Zenta payments into your product in minutes with our developer-first REST API." },
];
...
const PRICING = [
  ...
  {
    name: "Growth", price: "$19", period: "per month",
...
const TESTIMONIALS = [
  { quote: "Zenta cut our international payroll time from 3 days to 20 minutes. It's the only tool we actually kept.", name: "Amara Osei", role: "CFO, BrightLayer Studios", avatar: "AO" },
  { quote: "As a freelancer billing clients in the US and EU, Zenta saves me hundreds of dollars a month in FX fees alone.", name: "Raj Mehta", role: "Independent Product Designer", avatar: "RM" },
  { quote: "The API documentation is genuinely good. We integrated Zenta into our checkout in an afternoon.", name: "Lena Fischer", role: "CTO, Shopmatic", avatar: "LF" },
];

const TICKER_ITEMS = [
  "USD → NGN  ₦1,612.40", "GBP → EUR  €1.1724", "USD → GHS  ₵15.82",
  "EUR → KES  KSh 142.30", "USD → ZAR  R18.64", "GBP → NGN  ₦2,035.10", "USD → EGP  E£49.12",
];
...
function MockCard() {
  ...
  return (
    <div style={{
      ...
      border: "1px solid #00ff8830", borderRadius: 16,
    ...
    <div style={{ background: "#080c08", color: "#e8ede8", minHeight: "100vh", fontFamily: "'Instrument Sans', sans-serif", overflowX: "hidden" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .feat-card { border: 1px solid #1a221a; border-radius: 12px; padding: 32px 28px; background: #0c100c; transition: border-color 0.3s ease, transform 0.3s ease, box-shadow 0.3s ease; }
        .feat-card:hover { border-color: #00ff8840; transform: translateY(-4px); box-shadow: 0 8px 32px #00ff8810; }

        .pricing-card { border: 1px solid #1a221a; border-radius: 16px; padding: 36px 28px; background: #0c100c; transition: transform 0.3s ease; position: relative; }
        .pricing-card:hover { transform: translateY(-4px); }
        .pricing-card.highlight { border-color: #00ff88; background: #0a130a; box-shadow: 0 0 40px #00ff8818; }

        .testi-card { border: 1px solid #1a221a; border-radius: 12px; padding: 28px 24px; background: #0c100c; }

        .btn-green { display: inline-block; background: #00ff88; color: #050905; padding: 14px 32px; border-radius: 8px; font-size: 14px; font-weight: 700; letter-spacing: 0.03em; cursor: pointer; border: none; font-family: inherit; transition: opacity 0.2s ease, transform 0.2s ease; text-decoration: none; white-space: nowrap; }
        .btn-green:hover { opacity: 0.88; transform: translateY(-1px); }

        .btn-ghost { display: inline-block; background: transparent; color: #e8ede8; padding: 13px 28px; border-radius: 8px; font-size: 14px; font-weight: 500; letter-spacing: 0.03em; cursor: pointer; border: 1px solid #2a362a; font-family: inherit; transition: border-color 0.2s ease, background 0.2s ease; text-decoration: none; white-space: nowrap; }
        ...
        .grid-bg {
          background-image: linear-gradient(#00ff8806 1px, transparent 1px), linear-gradient(90deg, #00ff8806 1px, transparent 1px);
        ...
        .nav-link { background: none; border: none; cursor: pointer; font-size: 13px; font-family: inherit; color: #8a9a8a; letter-spacing: 0.06em; text-transform: uppercase; font-weight: 500; transition: color 0.2s ease; padding: 4px 0; }
        .nav-link:hover { color: #e8ede8; }
        .check { color: #00ff88; margin-right: 10px; }

        .z-section { padding: 100px 48px; max-width: 1160px; margin: 0 auto; }
        .z-nav-pad { padding: 0 48px; }
        .z-footer-pad { padding: 36px 48px; }
        ...
        .z-desktop-nav { display: flex; align-items: center; gap: 36px; }
        .z-hamburger { display: none; }

        @media (max-width: 768px) {
          .z-section { padding: 64px 20px; }
          .z-nav-pad { padding: 0 20px; }
          .z-footer-pad { padding: 28px 20px; flex-direction: column; align-items: flex-start; gap: 16px; }
          ...
          .z-cta-inner { padding: 48px 24px !important; }
          .z-cta-email-row { flex-direction: column !important; align-items: stretch !important; }
          .z-cta-email-row input { width: 100% !important; }
        }
        ...
        @media (max-width: 480px) {
          .z-section { padding: 56px 16px; }
          .z-nav-pad { padding: 0 16px; }
        }
      `}</style>

      <nav className="z-nav-pad" style={{
        ...
        <button className="z-hamburger" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu"
          style={{ background: "none", border: "none", cursor: "pointer", padding: 6 }}>
          <div style={{ width: 24, display: "flex", flexDirection: "column", gap: 5 }}>
            <span style={{ height: 1.5, background: "#e8ede8", display: "block", transition: "all 0.3s", transform: menuOpen ? "rotate(45deg) translateY(6.5px)" : "none" }} />
            <span style={{ height: 1.5, background: "#e8ede8", display: "block", transition: "all 0.3s", opacity: menuOpen ? 0 : 1 }} />
            <span style={{ height: 1.5, background: "#e8ede8", display: "block", transition: "all 0.3s", transform: menuOpen ? "rotate(-45deg) translateY(-6.5px)" : "none" }} />
          </div>
        </button>
        ...
        {menuOpen && (
          <div style={{
            ...
            background: "#080c08", borderTop: "1px solid #1a221a",
            padding: "24px 20px", display: "flex", flexDirection: "column", zIndex: 99,
          }}>
            {NAV_LINKS.map(l => (
              <button key={l} onClick={() => scrollTo(l.toLowerCase())}
                style={{ background: "none", border: "none", cursor: "pointer", fontSize: 24, letterSpacing: "0.04em", textTransform: "uppercase", fontFamily: "'Syne', sans-serif", textAlign: "left", color: "#e8ede8", fontWeight: 700, padding: "18px 0", borderBottom: "1px solid #1a221a" }}>
                {l}
              </button>
            ))}
            <div style={{ marginTop: 32 }}>
              <button className="btn-green" onClick={() => setMenuOpen(false)} style={{ width: "100%", textAlign: "center", padding: "16px" }}>Get Started</button>
            </div>
          </div>
        )}
      </nav>

      <section className="grid-bg noise z-section" style={{ position: "relative", paddingTop: 140, paddingBottom: 100 }}>
        ...
        <div className="hero-grid" style={{ position: "relative" }}>
          <div>
            ...
            <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(2.4rem, 7vw, 4.2rem)", fontWeight: 800, lineHeight: 1.08, letterSpacing: "-0.03em", marginBottom: 24 }}>
              Move Money<br />
              <span style={{ color: "#00ff88" }}>Anywhere.</span><br />
              Instantly.
            </h1>

            <p style={{ fontSize: "clamp(15px, 2vw, 17px)", lineHeight: 1.75, color: "#7a8a7a", fontWeight: 300, maxWidth: 420, marginBottom: 40 }}>
            ...
            <div className="hero-stats">
              {[["$2.4B+", "Processed monthly"], ["180ms", "Avg transfer time"], ["99.99%", "Uptime SLA"]].map(([val, label]) => (
                <div key={label}>
                  <p style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(18px, 3vw, 22px)", fontWeight: 700, color: "#e8ede8", letterSpacing: "-0.02em" }}>{val}</p>
      ...
      <section id="features" ref={featRef} className="z-section">
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          <p className={`z-fade d1 ${featInView ? "in" : ""}`} style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#00ff88", marginBottom: 16, fontWeight: 600 }}>Why Zenta</p>
          <h2 className={`z-fade d2 ${featInView ? "in" : ""}`} style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1 }}>
      ...
      <section id="pricing" ref={pricingRef} className="z-section">
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          <p className={`z-fade d1 ${pricingInView ? "in" : ""}`} style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#00ff88", marginBottom: 16, fontWeight: 600 }}>Pricing</p>
          <h2 className={`z-fade d2 ${pricingInView ? "in" : ""}`} style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1 }}>
      ...
      <section id="testimonials" ref={testimonialsRef} className="z-section">
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          <p className={`z-fade d1 ${testimonialsInView ? "in" : ""}`} style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#00ff88", marginBottom: 16, fontWeight: 600 }}>Testimonials</p>
          <h2 className={`z-fade d2 ${testimonialsInView ? "in" : ""}`} style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1 }}>
      ...
      <section ref={ctaRef} className="z-section" style={{ paddingBottom: 120 }}>
        <div className={`z-fade d1 ${ctaInView ? "in" : ""}`}
          style={{ background: "linear-gradient(135deg, #0d1a0d 0%, #0a120a 100%)", border: "1px solid #00ff8830", borderRadius: 20, textAlign: "center", position: "relative", overflow: "hidden", boxShadow: "0 0 80px #00ff8810" }}
        >
          <div className="z-cta-inner" style={{ padding: "80px 60px" }}>
            ...
            <p style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#00ff88", marginBottom: 20, fontWeight: 600, position: "relative" }}>Get started today</p>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 4vw, 3.2rem)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: 20, position: "relative" }}>
              The world doesn't wait.<br />Your payments shouldn't either.
            </h2>
            <p style={{ fontSize: 16, color: "#6a7a6a", fontWeight: 300, marginBottom: 40, maxWidth: 420, margin: "0 auto 40px", position: "relative" }}>
              Join 40,000+ businesses already moving money smarter with Zenta.
            </p>

            {submitted ? (
              <p style={{ color: "#00ff88", fontWeight: 600, fontSize: 16, position: "relative" }}>✓ You're on the list — we'll be in touch!</p>
            ) : (
              <div className="z-cta-email-row" style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", position: "relative" }}>
                <input
                  type="email"
                  placeholder="Enter your work email"
                  value={email}
                  onChange={e => setEmail(e.target.value)}
                  style={{ background: "#ffffff08", border: "1px solid #2a362a", borderRadius: 8, padding: "13px 20px", fontSize: 14, color: "#e8ede8", fontFamily: "inherit", outline: "none", width: 280, maxWidth: "100%" }}
                />
      ...
      <footer className="z-footer-pad" style={{ borderTop: "1px solid #1a221a", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
